...
Taxon = "Clausocalanus" # Regex pattern to filter taxonomy
...
Load hierarchical taxonomic classification for species IDs. The classification dataset was made with the SO_CPR_AnnotateSpeciesTaxonomy.R script. Change the file path accordingly.

```{r Load Taxonomic Metadata, include = TRUE}
## Read taxonomic metadata for every ID ####
taxonomy <- read.table(
  file = "C:/Users/pdeschepper/Desktop/ADVANCE/VRI/Rcode/Krill_Rshiny/SO-CPR_ID_TaxonAnnotations.txt",
  sep = "\t", 
  header = TRUE
)
```
...
## 4.3 Temporal Trends
Plot annual abundance patterns for selected taxa.
...
ggplot(species_presence_yearly, aes(x = Year, y = Corrected_Abundance, group = Species, color = Species)) +
  geom_line() +
  geom_point() +
  labs(title = "Yearly Corrected Abundance of Species",
       x = "Year",
...
Boxplot showing monthly variation in total abundance.
